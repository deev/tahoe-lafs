...
class MutableTest(unittest.TestCase):
    ...
    def test_dont_repeat_your_question(self):
        """
        If a mutable filenode is asked to modify the contents, then it should
        not ask for a servermap more than once. Just ask once and then work
        with what you get.
        ...
        This test was suggested by Daira.
        """
        class MockServerMap(object):
            def __init__(self, *args, **kwargs):
                self.how_many_times_asked = 0
            ...
            def best_recoverable_version(self):
                self.failUnlessEqual(self.how_many_times_asked, 0)
                verinfo = verinfos[self.how_many_times_asked]
                self.how_many_times_asked += 1
                return verinfo
